      ...
      <div className={`w-full max-w-4xl grid md:grid-cols-2 rounded-3xl overflow-hidden shadow-2xl ${dark ? "bg-slate-900" : "bg-white"}`}>
        ...
        <form onSubmit={submit} className="p-8 md:p-10 flex flex-col justify-center">
          ...
          <button type="button" onClick={submit}
            className={`w-full rounded-xl py-2.5 font-medium border flex items-center justify-center gap-2 ${dark ? "border-slate-700 text-gray-200" : "border-gray-200 text-gray-700"}`}>
            <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" /><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.25 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.85A11 11 0 0012 23z" /><path fill="#FBBC05" d="M5.84 14.1A6.6 6.6 0 015.5 12c0-.73.13-1.44.34-2.1V7.05H2.18A11 11 0 001 12c0 1.77.43 3.45 1.18 4.95l3.66-2.85z" /><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1a11 11 0 00-9.82 6.05l3.66 2.85C6.71 7.3 9.14 5.38 12 5.38z" /></svg>
            {t.signInGoogle}
          </button>
...
/* ============================== LAYOUT ============================== */
function Sidebar({ t, page, setPage, dark, lang, mobileOpen, setMobileOpen }) {
  const items = [
...
/* ============================== COMMENTS PAGE ============================== */
function CommentsPage({ t, dark, comments, setComments, lang }) {
                ...
                <tr key={c.id} className={`border-t ${dark ? "border-slate-800" : "border-gray-100"}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      ...
                      <button onClick={() => del(c.id)} className={`p-1.5 rounded-lg ${dark ? "hover:bg-slate-800 text-rose-400" : "hover:bg-gray-100 text-rose-500"}`}><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
...
function UsersPage({ t, dark }) {
  const [users, setUsers] = useState([
    { id: 1, name: "Admin User", email: "admin@hajj.sa", role: "admin" },
    { id: 2, name: "Sara Analyst", email: "sara@hajj.sa", role: "manager" },
    { id: 3, name: "Guest Viewer", email: "guest@hajj.sa", role: "viewer" },
  ]);
  return (
    <div className="p-4 md:p-6 space-y-4">
      <Card dark={dark} className="p-4 flex justify-between items-center">
        ...
        <button onClick={() => setUsers((u) => [...u, { id: Date.now(), name: "New User", email: "new@hajj.sa", role: "viewer" }])}
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className={`border-t ${dark ? "border-slate-800" : "border-gray-100"}`}>
                ...
                <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${dark ? "bg-slate-800 text-emerald-300" : "bg-emerald-50 text-emerald-700"}`}>{t[u.role]}</span></td>
                <td className="px-4 py-3">
                  <div className="flex gap-1.5">
                    ...
                    <button onClick={() => setUsers((list) => list.filter((x) => x.id !== u.id))} className={`p-1.5 rounded-lg ${dark ? "hover:bg-slate-800 text-rose-400" : "hover:bg-gray-100 text-rose-500"}`}><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
...
export default function HajjUmrahSystem() {
  ...
  if (!loggedIn) {
    return (
      <div dir={lang === "ar" ? "rtl" : "ltr"} style={{ fontFamily: fontStack }}>
        ...
        <LoginPage t={t} lang={lang} setLang={setLang} dark={dark} setDark={setDark} onLogin={() => setLoggedIn(true)} />
      </div>
    );
  }
  ...
  const pageComp = {
    ...
    comments: <CommentsPage t={t} dark={dark} comments={comments} setComments={persist} lang={lang} />,
    analytics: <AnalyticsPage t={t} dark={dark} comments={comments} />,
    reports: <ReportsPage t={t} dark={dark} comments={comments} />,
    users: <UsersPage t={t} dark={dark} />,
  ...
  return (
    <div dir={lang === "ar" ? "rtl" : "ltr"} style={{ fontFamily: fontStack }} className={dark ? "dark" : ""}>
      ...
      <div className={`min-h-screen flex ${dark ? "bg-slate-950" : "bg-gray-50"}`}>
        <Sidebar t={t} page={page} setPage={setPage} dark={dark} lang={lang} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
